...
**Depends On**: [INCREMENT_21D.md](./INCREMENT_21D.md)
**Focus**: IntegrationView content page with educational material and widget integration

---

## Overview

This increment creates the content page for integration at `/calculus/integration`. The page follows the design system patterns (three-analogy block, pitfall callouts, CodeExample with IDs) and embeds the IntegrationExplorer widget.

---

## Tasks

### Task 1: Create IntegrationView Component

**File**: `src/views/calculus/IntegrationView.vue` (new)
...
### Task 2: Add Route to Router

**File**: `src/router/index.ts` (update)

Add the integration route. Find the calculus routes section and add:
...
### Task 3: Update Calculus Index Page

**File**: `src/views/calculus/CalculusIndexView.vue` (update)

Change the Integration topic from `available: false` to `available: true`:

Find this in the topics array:
```ts
{
  title: 'Integration',
  path: '/calculus/integration',
...
### Task 4: Update Navigation Data (if needed)

**File**: `src/data/navigation.ts` (check/update)

If navigation data includes calculus subtopics, ensure integration is listed:
...
```ts
{
  ...
  title: 'Integration',
  path: '/calculus/integration',
  description: 'Area under curves is just summation with infinitely small steps',
},
```
...
| File | Action | Status |
|------|--------|--------|
| `src/views/calculus/IntegrationView.vue` | Create new | ⬜ |
| `src/router/index.ts` | Add route | ⬜ |
| `src/views/calculus/CalculusIndexView.vue` | Update available flag | ⬜ |
| `src/data/navigation.ts` | Check/update | ⬜ |

---

## Success Criteria

- [ ] Page compiles without errors (`npm run type-check`)
- [ ] No ESLint errors (`npm run lint`)
- [ ] Page accessible at `/calculus/integration`
- [ ] Three-analogy block present with correct styling
- [ ] Pitfall callout present with amber styling
- [ ] IntegrationExplorer widget renders and works
- [ ] All CodeExample components have unique IDs
- [ ] RelatedTopics links work
- [ ] Dark mode renders correctly
- [ ] Calculus index page shows Integration as available (not "Coming Soon")
...
- [ ] Three-analogy block (amber/emerald/blue cards with `bg-surface-alt`)
- [ ] Common pitfall callout (amber warning box)
- [ ] CodeExample with `id` prop (format: `calculus-integration-{descriptor}`)
- [ ] CodeExample with `title` prop (filename.py style)
- [ ] CollapsiblePanel for advanced sections (`:default-expanded="false"`)
- [ ] RelatedTopics at end (4 items including parent index)
- [ ] Dark mode variants for colored text

---

## Content Verification

- [ ] Introduction explains integrals as area under curves
- [ ] Signed area concept explained clearly
- [ ] Widget "Try These Experiments" section guides exploration
- [ ] Riemann sum methods compared with convergence rates
- [ ] FTC explained with both parts
- [ ] Pitfalls cover +C, signed area, bound swap, Simpson's odd n
- [ ] Python section shows scipy.integrate.quad and numpy.trapz

---

## Notes

- The content page is quite long; ensure all sections load properly
- CodeExample IDs follow the pattern `calculus-integration-{descriptor}`
- The IntegrationExplorer widget uses `sync-url` to enable shareable links
- Related Topics includes cross-section link to Summation (algebra section)
