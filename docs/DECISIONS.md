...
**Decision**: Name the matrix widget "MatrixTransformations" to focus on the visual transformation aspect.

**Rationale**:
- Emphasizes the visual/geometric interpretation over pure computation
- Aligns with educational goal of making matrices intuitive
- "Transformations" is more approachable than "Operations"
- Follows pattern of descriptive widget names (QuadraticExplorer, VectorOperations)

---

### D-097: 2×2 Matrices Only (No Higher Dimensions)
**Decision**: Limit matrix visualization to 2×2 matrices for 2D transformations.

**Rationale**:
- 2D transformations can be visualized clearly in a planar coordinate system
- 3D transformations require complex rendering (perspective, rotation controls)
- Core concepts (rotation, scale, shear, reflection) are fully demonstrable in 2D
- Keeps widget focused on understanding, not complexity
- 3×3 or higher can be added in a future phase if needed

**Trade-off**: Cannot demonstrate 3D transformations or homogeneous coordinates. Acceptable for educational purposes.

---

### D-098: Unit Square as Primary Visualization
**Decision**: Show a unit square (vertices at origin, (1,0), (1,1), (0,1)) being transformed.

**Rationale**:
- Unit square provides clear reference for understanding transformation effects
- Shows area change (determinant) geometrically
- Reveals shear, rotation, reflection effects intuitively
- Standard educational approach in linear algebra courses
- Original square shown dashed, transformed shown filled

**Implementation**:
- Original: dashed outline at unit square position
- Transformed: filled polygon at new positions
- Both visible simultaneously for comparison

---

### D-099: Basis Vectors with Arrow Markers
**Decision**: Display î (1,0) and ĵ (0,1) basis vectors with colored arrows.

**Rationale**:
- Matrices transform basis vectors - showing this is educational
- Color coding (blue for î, amber for ĵ) aids visual tracking
- Arrow markers using SVG `<marker>` elements
- Shows both original (dashed) and transformed (solid) vectors

**Colors**:
- î (x-axis unit vector): Blue (#3b82f6)
- ĵ (y-axis unit vector): Amber (#f59e0b)

---

### D-100: Ten Transformation Types
**Decision**: Support 10 transformation types: identity, rotation, scale, uniformScale, shearX, shearY, reflectX, reflectY, reflectOrigin, custom.

**Rationale**:
- Covers all fundamental 2D linear transformations
- Each type has distinct geometric meaning
- "Custom" allows advanced users to enter arbitrary matrices
- Named types provide educational scaffolding
...
| scale | scaleX, scaleY |
| uniformScale | scaleX (controls both) |
| shearX | shear factor |
| shearY | shear factor |
| reflectX, reflectY, reflectOrigin | None |
| custom | 4 matrix entries (a, b, c, d) |

---

### D-101: Determinant-Based Property Badges
**Decision**: Display badges for matrix properties computed from the determinant.

**Rationale**:
- Determinant encodes geometric information (area scaling, orientation)
- Badges provide immediate visual feedback
- Educational: teaches what determinant values mean

**Badge Logic**:
| Condition | Badge | Meaning |
|-----------|-------|---------|
| det = 1 | Area Preserving | Rotation, reflection |
| det < 0 | Orientation Reversing | Includes reflection |
| det ≈ 0 | Singular | Collapses to line/point |
| Orthogonal | Orthogonal | Preserves lengths/angles |

---

### D-102: Composable Pattern for Matrix State (useMatrixTransformations)
**Decision**: Create `useMatrixTransformations` composable following established patterns.

**Rationale**:
- Consistent with useVectors, useUnitCircle, useStatistics patterns
- Separates state logic from presentation
- Supports optional URL state synchronization
- Computed properties handle derived values (currentMatrix, determinant, etc.)
- Testable independently of components

**Implementation**:
```typescript
export function useMatrixTransformations(options: UseMatrixTransformationsOptions = {}) {
  // State: transformationType, angle, scaleX, scaleY, shear, customMatrix
  // Computed: currentMatrix, currentDeterminant, isCurrentOrthogonal, transformedSquare
  // Methods: setTransformationType, setAngle, setScale, loadPreset
  // URL sync: optional via options.syncUrl
}
```
...
**Decision**: Build MatrixTransformations from discrete sub-components.

**Rationale**:
- Single responsibility per component
- Easier to test and maintain
- Components can be reused in other contexts
- Consistent with VectorOperations, StatisticsCalculator patterns

**Structure**:
```
MatrixTransformations/ (orchestrator)
├── TransformationCanvas.vue (SVG visualization)
├── TransformationControls.vue (type buttons, sliders)
├── MatrixDisplay.vue (2×2 matrix with edit mode)
├── TransformInfo.vue (determinant, properties, badges)
└── index.ts (exports)
```
...
**Decision**: Provide preset configurations with descriptive names for common transformations.

**Rationale**:
- Quick access to educational examples
- Names like "90° Rotation" are more approachable than raw matrices
- Demonstrates variety of transformation effects
- URL-shareable via preset IDs

**Presets**:
| ID | Name | Type | Parameters |
|----|------|------|------------|
| rotate-90 | 90° Rotation | rotation | angle: 90 |
| rotate-45 | 45° Rotation | rotation | angle: 45 |
| scale-2x | Double Size | uniformScale | k: 2 |
| scale-half | Half Size | uniformScale | k: 0.5 |
| flip-x | Flip Horizontal | reflectY | - |
| flip-y | Flip Vertical | reflectX | - |
| shear-right | Shear Right | shearX | k: 0.5 |
| shear-up | Shear Up | shearY | k: 0.5 |

---

### D-105: URL Parameters Use Short Names
**Decision**: Use short parameter names in URL (sx, sy, type, angle, shear) rather than verbose names.

**Rationale**:
- Keeps URLs shorter and cleaner
- Still readable (sx = scale X, sy = scale Y)
- Consistent with established pattern from other composables
