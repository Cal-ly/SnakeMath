# Increment 12D: Matrices Content Page

**Goal**: Create the comprehensive content page with widget integration, code examples, and educational content.

**Estimated Time**: 45 minutes

**Prerequisites**: Increment 12C complete (all widget components functional)

---

## Files to Create/Update

### 1. `src/views/linear-algebra/MatricesView.vue`

Full content page following the pattern from `VectorsView.vue`:
...
Add the matrices route after the vectors route:

```typescript
// In the routes array, after the vectors route:
{
...
### 3. Update `src/data/navigation.ts`

Add matrices to the linear algebra subtopics:

```typescript
// In the linear-algebra topic's subtopics array:
{
  id: 'matrices',
  title: 'Matrices',
...
### 4. Update `src/views/linear-algebra/LinearAlgebraIndexView.vue`

The index page should already pick up the new subtopic automatically from navigation data, but verify it displays correctly.
...
- [ ] What is a matrix (grid of numbers, transformation interpretation)
- [ ] Interactive widget with URL sync
- [ ] Transformation types (rotation, scale, shear, reflection)
- [ ] Mathematical formulas for each transformation
- [ ] The determinant (area scaling, orientation)
- [ ] Composition of transformations (order matters)
- [ ] Inverse matrices (undoing transformations)
- [ ] Applications (graphics, neural networks, image processing)
- [ ] Python code examples for each section
...
- [ ] `/linear-algebra/matrices` route works
- [ ] Navigation includes Matrices link
- [ ] Page renders all sections correctly
- [ ] Widget is visible and functional
- [ ] URL sync works (change transformation, refresh, state preserved)
...
# Navigate to /linear-algebra/matrices and verify:
# 1. Page loads
# 2. Widget works
# 3. All sections expand/collapse
# 4. Code examples render
# 5. Links work
