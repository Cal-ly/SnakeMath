...
- **Math rendering**: KaTeX (smaller, faster than MathJax)
- **Syntax highlighting**: Shiki (VSCode-quality)
- **CodeExample features**: Copy, line numbers (optional), title, collapsible
- **SymbolTable layout**: Responsive hybrid (table on desktop, cards on mobile)
- **Copy feedback**: Subtle icon change with brief highlight animation
- **TabGroup API**: Hybrid (props for labels, slots for content)
- **Symbol data**: Split by category for maintainability
