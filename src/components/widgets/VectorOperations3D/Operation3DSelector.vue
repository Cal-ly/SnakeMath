...
import type { Vector3DOperation } from '@/types/math'

interface Props {
  selectedOperation: Vector3DOperation
}

defineProps<Props>()

const emit = defineEmits<{
  select: [operation: Vector3DOperation]
}>()

const operations: Array<{ id: Vector3DOperation; label: string; icon: string; description: string }> = [
  { id: 'add', label: 'Add', icon: '+', description: 'A + B' },
  { id: 'subtract', label: 'Subtract', icon: '−', description: 'A − B' },
  { id: 'cross', label: 'Cross Product', icon: '×', description: 'A × B' },
...
function handleSelect(op: Vector3DOperation) {
  emit('select', op)
}
</script>

<template>
  <div class="operation-selector" data-testid="operation-3d-selector">
    <h3 class="text-sm font-semibold text-text-muted uppercase tracking-wide mb-3">
      Operation
    </h3>
    <div class="flex flex-wrap gap-2" role="radiogroup" aria-label="Select 3D vector operation">
      <button
